...
- A column's sort key, filter values and clipboard content all come from [`OperationContentBinding`](xref:WinUI.TableView.TableViewColumn.OperationContentBinding), which falls back to `Binding` on a bound column — **including its converter**. Put display-only conversion (glyphs, formatted or localized text) in the cell template or a custom column's `GenerateElement`, not on `Binding`, or the converted value becomes the sort and filter key too. To change only how filter values are *displayed*, use [`FilterItemTemplate`](xref:WinUI.TableView.TableViewColumn.FilterItemTemplate) — see [Filtering](filtering.md).
- A custom column type derives from [`TableViewBoundColumn`](xref:WinUI.TableView.TableViewBoundColumn) (or an existing concrete column) and overrides `GenerateElement` / `GenerateEditingElement`. Note that every built-in column applies `Margin="12,0,12,0"` to the element it generates; a custom column chooses its own, and should not be countered with negative margins from a consumer style — `TableViewCell.MeasureOverride` collapses the content presenter when the resulting content box is negative.
